...
const (
	appLogo = "UDP tunneling from ZIVPN"
	appDesc    = "UDP tunneling from ZIVPN"
	appAuthors = "ZI"

	appLogLevelEnv           = "ZIVPN_LOG_LEVEL"
	appLogFormatEnv          = "ZIVPN_LOG_FORMAT"
	appACMEDirEnv            = "ZIVPN_ACME_DIR"
)
...
var (
	...
	appVersionLong = fmt.Sprintf("Version:\t%s\n"+
		"BuildDate:\t%s\n"+
		"Authors:\t%s",
		appVersion, appDate, appAuthor)

	appAboutLong = fmt.Sprintf("%s\n%s", appLogo, appVersionLong)
)
...
func Execute() {
	...
	clientCmd.SetHelpFunc(helpFunc)
	serverCmd.SetHelpFunc(helpFunc)
	pingCmd.SetHelpFunc(helpFunc)
	statsCmd.SetHelpFunc(helpFunc)
	err := rootCmd.Execute()
	if err != nil {
		os.Exit(1)
	}
...
func initFlags() {
	...
	rootCmd.PersistentFlags().StringVar(&signature, "signature", "", "signature")
...
func initConfig() {
	if cfgFile != "" {
		...
		viper.AddConfigPath(".")
		viper.AddConfigPath("$HOME/.zivpn")
		viper.AddConfigPath("/etc/zivpn/")
	}
}
